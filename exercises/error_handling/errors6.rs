...
fn parse_pos_nonzero3(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
...
fn parse_pos_nonzero_ORIG(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
    // TODO: change this to return an appropriate error instead of panicking
    // when `parse()` returns an error.
    let x: i64 = s.parse().unwrap();
    PositiveNonzeroInteger::new(x).map_err(ParsePosNonzeroError::from_creation)
...
fn parse_pos_nonzero2(s: &str) -> Result<PositiveNonzeroInteger, ParsePosNonzeroError> {
    // TODO: change this to return an appropriate error instead of panicking
    // when `parse()` returns an error.
    let y = s.parse().unwrap();
/*     if !y.is_ok() {
        return ParsePosNonzeroError::from_parseint;
    } */
    //let x: i64 = y;
 /*    if (!x.is_ok()) {
        return ParsePosNonzeroError::ParseInt(ParseIntError);
    } */
